...
export default function DateStrip({ value, onChange, days = 16 }: DateStripProps) {
  ...
  return (
    <Box role="group" aria-label="Trip date"
      sx={{ display: 'flex', gap: { xs: 0.45, md: 0.6 }, overflowX: 'auto', pb: { xs: 0.25, md: 0.5 }, scrollSnapType: 'x proximity',
        ...
        return (
          <Box key={iso} role="button" tabIndex={0} aria-pressed={sel} aria-label={iso}
            ...
            sx={{ flex: '0 0 auto', scrollSnapAlign: 'start', cursor: 'pointer', userSelect: 'none',
              minWidth: { xs: 42, md: 44 }, px: { xs: 0.85, md: 1 }, py: { xs: 0.48, md: 0.6 }, borderRadius: '10px', textAlign: 'center',
            ...
            <Typography sx={{ fontSize: { xs: '0.56rem', md: '0.6rem' }, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.03em', lineHeight: 1.35, color: sel ? 'primary.contrastText' : 'text.secondary', opacity: sel ? 0.72 : 1 }}>{wd}</Typography>
            <Typography sx={{ fontSize: { xs: '0.86rem', md: '0.92rem' }, fontWeight: 700, lineHeight: 1.08 }}>{d}</Typography>
          </Box>
        );
      })}
    </Box>
