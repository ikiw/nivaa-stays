...
export default function AiBar({ isMobile, query, setQuery, onPlan, busy, autoFocus }: AiBarProps) {
  return (
    <Paper elevation={0} sx={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 0.5, width: '100%', px: 0.55, py: 0.32, borderRadius: 999,
      backgroundColor: ACTIVE.mode === 'light' ? 'rgba(255,255,255,0.5)' : 'rgba(18,19,22,0.42)', backdropFilter: 'blur(14px)',
      boxShadow: '0 4px 18px rgba(0,0,0,0.34)', transition: 'box-shadow .18s ease',
      '&::before': { content: '""', position: 'absolute', inset: 0, borderRadius: 'inherit', padding: '1.5px', pointerEvents: 'none',
        background: ACTIVE.mode === 'light' ? '#C9A227' : '#2A3040',
      ...
      <TextField fullWidth variant="standard" autoFocus={autoFocus} placeholder={isMobile ? 'Describe your ideal day…' : 'Describe your ideal day…'}
        value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') onPlan(); }}
        InputProps={{ disableUnderline: true, startAdornment: <AutoAwesomeRounded sx={{ color: 'secondary.main', ml: 0.75, mr: 1 }} />, sx: { fontSize: '0.88rem' } }} />
      <Button variant="contained" onClick={onPlan} disabled={busy} aria-label="Plan my day"
        ...
        sx={{ borderRadius: 999, flexShrink: 0, minWidth: isMobile ? 44 : undefined, px: isMobile ? 0 : 2,
          bgcolor: '#E6C35A', color: '#2A2100', boxShadow: 'none', fontWeight: 850, '&:hover': { bgcolor: '#D9B441', boxShadow: 'none' },
